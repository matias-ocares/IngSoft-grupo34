...
class solicitud_aprobada_enviada extends controller {
    ...
    private function set_config() { //seteo la configuración 
        $array_estados = array(2);  //estados por los que filtro (aprobado)
        //Base properties
        $config['base_url'] = 'http://localhost:1234/IngSoft-grupo34/Aventon/index.php/solicitud_aprobada_enviada/';
    ...
    public function index($rowno = 0) {
        ...
        if ($this->model_solicitud-> tiene_una_solicitud()) {
            ...
            foreach ($lista_solicitudes as $solicitud) {
                ...
                //$this->table->add_row($solicitud['origen'], $solicitud['destino'], $newDate, $hora_inicio, $solicitud['nombre'] . ", " . $solicitud['apellido'], anchor('', 'Darme de Baja'));
                $this->table->add_row($solicitud['id_estado'], $solicitud['id_user'], $solicitud['id_viaje'], anchor('', 'Darme de Baja'));
            }
            ...
            $this->session->set_flashdata('listado','Aprobadas Enviadas');
